...
// Batch item: parent_id present → update existing, absent → create new
export const batchParentItemSchema = Joi.object({
...
export const batchParentsSchema = Joi.object({
  parents: Joi.array().items(batchParentItemSchema).min(1).max(2).required(),
});
...
export const createStudentSchema = Joi.object({
  ...
  parents: Joi.array().items(parentInputSchema).max(2).optional(),
});
...
export const updateStudentSchema = Joi.object({
  ...
  // grade_level removed in sprint 1 — tied to class, handled via ClassTransferRequest (sprint 2)
  enrollment_date: Joi.string().isoDate().allow(null, '').optional(),
  ethnicity: Joi.string().max(50).allow(null, '').optional(),
  nationality: Joi.string().max(50).allow(null, '').optional(),
  religion: Joi.string().max(50).allow(null, '').optional(),
  ...
  hometown_ward: Joi.string().max(100).allow(null, '').optional(),
  photo_url: Joi.string().max(500).allow(null, '').optional(),
  // class_id removed in sprint 1 — class transfer handled by ClassTransferRequest (sprint 2)
}).min(1);
